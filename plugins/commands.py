...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
                    ...
                    InlineKeyboardButton('Mᴏꜱᴛ Sᴇᴀʀᴄʜ 🔎', callback_data="topsearch"),
    ...
    if message.command[1].startswith("reff_"):
        ...
        if fromuse == 100:
            ...
            await client.send_message(user_id, f"𝗖𝗼𝗻𝗴𝗿𝗮𝘁𝘂𝗹𝗮𝘁𝗶𝗼𝗻𝘀! 𝗬𝗼𝘂 𝘄𝗼𝗻 𝟭𝟬 𝗥𝗲𝗳𝗲𝗿𝗿𝗮𝗹 ᴘᴏɪɴᴛ 𝗯𝗲𝗰𝗮𝘂𝘀𝗲 𝗬𝗼𝘂 𝗵𝗮𝘃𝗲 𝗯𝗲𝗲𝗻 𝗦𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆 𝗜𝗻𝘃𝗶𝘁𝗲𝗱 ☞{message.from_user.mention}!")
        return
        ...
        try:
            ...
            if btn:
                ...
                fsub_msg_text = (
                    ...
                    "Iғ ʏᴏᴜ ᴡᴀɴᴛ ᴛʜᴇ ᴍᴏᴠɪᴇ ғɪʟᴇ, ᴊᴏɪɴ ᴏᴜʀ ʙᴀᴄᴋ-ᴜᴘ ᴄʜᴀɴɴᴇʟ, ᴛʜᴇɴ ᴄʟɪᴄᴋ ᴏɴ ᴛʜᴇ '↻ Tʀʏ Aɢᴀɪɴ' ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ..."
                )
                
                # Using reply_text instead of reply_photo
...
async def topsearch_callback(client, callback_query):
    ...
    if not truncated_messages:
        return await callback_query.answer("Nᴏ sᴇᴀʀᴄʜᴇs ʀᴇᴄᴏʀᴅᴇᴅ ʏᴇᴛ!", show_alert=True)

    # Neat numbered list with Click-to-Copy formatting
    list_text = "\n".join([f"⚡️ {i+1}. <code>{msg}</code>" for i, msg in enumerate(truncated_messages)])
    
    message_text = (
        "<b>📊 <u>Mᴏsᴛ Sᴇᴀʀᴄʜᴇᴅ Tᴏᴅᴀʏ</u></b>\n\n"
        f"{list_text}\n\n"
        "💡 <i>Tip: Click on the movie name to copy it instantly!</i>"
    )
    
    # Inline Close Button
    btn = [[InlineKeyboardButton("🛑 ᴄʟᴏsᴇ ᴍᴇɴᴜ 🛑", callback_data="close_data")]]
...
async def top(_, message):
    ...
    if not truncated_messages:
        return await message.reply_text("Nᴏ sᴇᴀʀᴄʜᴇs ʀᴇᴄᴏʀᴅᴇᴅ ʏᴇᴛ!")
...
async def trendlist(client, message):
    ...
    additional_message = (
        "⚡️ 𝑨𝒍𝒍 𝒕𝒉𝒆 𝒓𝒆𝒔𝒖𝒍𝒕𝒔 𝒂𝒃𝒐𝒗𝒆 𝒄𝒐𝒎𝒆 𝒇𝒓𝒐𝒎 𝒘𝒉𝒂𝒕 𝒖𝒔𝒆𝒓𝒔 𝒉𝒂𝒗𝒆 𝒔𝒆𝒂𝒓𝒄ʜ𝒆𝒅 𝒇𝒐𝒓. "
...
async def confirmation_handler(client, callback_query):
    ...
    if action == "yes":
        ...
        await callback_query.message.edit_text('🧹 ᴜᴘᴅᴀᴛᴇꜱ ᴄʜᴀɴɴᴇʟ ʟɪsᴛ ʜᴀs ʙᴇᴇʀ ᴄʟᴇᴀʀᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ ✅')
    elif action == "no":
